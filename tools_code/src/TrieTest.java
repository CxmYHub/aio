...
public class TrieTest {
    ...
    private static void testBasicOperations() {
        ...
        assertEqual(added, 5, "插入 hello 新增节点数");
    ...
    private static void testRemoveWithDepthBug() {
        ...
        //      同时 A 还有另一个子节点 X，对应长单词 "AXYZ" (长度4，depth分别为 4,3,2,1)
        trie t = new trie();
        t.input("ABCDEF");   // 长度6
        t.input("AXYZ");     // 长度4
        ...
        // 重新获取节点 A（因删除可能导致其子节点变化）
        nodeA = t.children['A' - 'A'];
        assertNotNull(nodeA, "删除后节点 A 仍存在");
        ...
        // 此时 A 应被删除（因为无其他子节点且自身不是单词结尾）
        trie rootAChild = t.children['A' - 'A'];
        assertEqual(rootAChild == null, true, "删除 AXYZ 后根节点的 A 子节点被清理");
    ...
    private static void testInputMoreAndCount() {
        ...
        // 预期新增节点：c(1), a(2), t(3), r(4), d(5), o(6), g(7) 共7个节点 + 3个单词结束标记（cat,car,dog）但 cat 已存在不计单词标记
        // 注意 input_more 返回值统计的是新增节点数，不包括重复单词的结束标记，但包括新单词的结束标记。
        // 具体：cat (3新节点+1结束) =4, car (1新节点'r'+1结束) =2, dog (3新节点+1结束) =4, cat重复 (0) => 总计10
        // 我们只验证返回值非负且基本合理，重点不在此。
        System.out.println("input_more 返回新增节点数: " + added);
