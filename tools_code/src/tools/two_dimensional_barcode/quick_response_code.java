...
public class quick_response_code
{
    ...
    */
    ...
    {
        System.out.println("纠错等级："+error_correction_level);
        int side=(version-1)*4+21;
        boolean[][] field=new boolean[side][side];
        if(version<1||version>40)
        {
            System.out.println("版本号错误，必须在1到40之间。");
            return field;
        }
    ...
    */
    public static boolean[][] encode(String text,int error_correction_level)
    {
        ...
        return encode(text,error_correction_level,version,mode);
    }
    /**
    通过文本计算二维码图形。<br>
    自动选择合适的编码模式和最小可用的版本。<br>
